...
<!-- If panel is null.. i.e. no entry in db then show below code.. -->
  ...
  <div class="columns is-multiline" id="content" v-if="panel.name_of_product==null">
    <div class="column is-5 only">
      <div class="field">
        <p class="control">
          <input v-validate="'required'" name="product_name" @keyup.alt.113="f2()" v-model="description" placeholder="Press F2" type="text" class="input">
        </p>
        <div v-show="errors.has('product_name')" class="help is-danger">
          {{ errors.first('product_name') }}
        </div>
...
export default {
  ...
  methods: {
    f2 () {
      ...
      this.description = 'GST Description 1'
      this.scode = 996711
